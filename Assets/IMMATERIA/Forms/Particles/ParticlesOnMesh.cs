...
public class ParticlesOnMesh : Particles {
  ...
  public override void Embody(){
    ...
    Vector4[] tans = mesh.sharedMesh.tangents;
    Vector3[] nors = mesh.sharedMesh.normals;
    ...
    for( int i = 0; i < count; i ++ ){
      ...
      if( transformed ){ nor = meshTransform.TransformDirection( nor ); }
      if( transformed ){ tan = meshTransform.TransformDirection( tan ); }
      if( transformed ){ pos = meshTransform.TransformPoint( pos ); }
